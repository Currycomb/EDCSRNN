...
parser.add_argument("--load_weights_dir", type=str, default='')
...
g = EDCSNN((patch_height, patch_width, input_channels))    # (128 * 128 * channels)
...
def Validate(iter, sample=0):
    ...
    validate_path = [x.replace('\\', '/') for x in validate_path]
    validate_path.sort()
    if sample == 1:    # 可视化
        r, c = 3, 3
        ...
        cnt = 0
        for row in range(r):
            axs[row, 1].set_title('MAE=%.4f, NRMSE=%.4f, SSIM=%.4f, PSNR=%.4f' % (np.sqrt(mses[row]), nrmses[row], ssims[row], psnrs[row]))
        ...
        mses, nrmses, psnrs, ssims = [], [], [], []
        val_nums = len(validate_path)

        for path in validate_path:
            # print(path, validate_images_path, validate_gt_path)
            [img, gt] = cur_data_loader([path], validate_images_path, validate_gt_path, patch_height,
                                        patch_width, 1, norm_flag=norm_flag, scale=scale_factor)
            output = np.squeeze(g.predict(img))
            mses, nrmses, psnrs, ssims = img_comp(gt, output, mses, nrmses, psnrs, ssims)
        ...
        if np.mean(maes) < min(validate_mae):
            g.save_weights(save_weights_path + 'weights.best')
            print('Best MAE: {}'.format(np.mean(maes)))

        validate_mae.append(np.mean(np.sqrt(mses)))
        curlr = lr_controller.on_epoch_end(iter, np.mean(nrmses))
        print(list(maes))
        print('Val MAE: {}'.format(np.mean(maes)))


if load_weights_dir:
    g.save_weights(load_weights_dir)
    print('Loading weights successfully: ' + save_weights_path + 'weights')
...
images_path = [x.replace('\\', '/') for x in images_path]
print(images_path[0])
for it in range(iterations):
    ...
    print("%d batches: time: %s, g_loss = %s" % (it + 1, elapsed_time, loss_generator))
